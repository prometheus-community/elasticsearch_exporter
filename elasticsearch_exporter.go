...
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	...
	for _, stats := range allStats.Nodes {
		...
		e.gauges["indices_segments_count"].Set(float64(stats.Indices.Segments.Count))
		...
		e.counters["indices_refresh_total_time_ms_total"].Set(float64(stats.Indices.Refresh.TotalTime))
		e.counters["indices_refresh_total"].Set(float64(stats.Indices.Refresh.Total))
		...
		// Process Stats
		e.gauges["process_cpu_percent"].Set(float64(stats.Process.CPU.Percent))
		e.gauges["process_mem_resident_size_bytes"].Set(float64(stats.Process.Memory.Resident))
		e.gauges["process_mem_share_size_bytes"].Set(float64(stats.Process.Memory.Share))
		e.gauges["process_mem_virtual_size_bytes"].Set(float64(stats.Process.Memory.TotalVirtual))
		e.gauges["process_open_files_count"].Set(float64(stats.Process.OpenFD))

		e.counterVecs["process_cpu_time_seconds_sum"].WithLabelValues("sys").Set(float64(stats.Process.CPU.Sys / 1000))
		e.counterVecs["process_cpu_time_seconds_sum"].WithLabelValues("user").Set(float64(stats.Process.CPU.User / 1000))
	}
