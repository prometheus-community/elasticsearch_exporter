...
Prometheus exporter for minimal metrics about ElasticSearch, written in Go.

Forked from https://github.com/prometheus-community/elasticsearch_exporter

The output of the original repo is an overkill. This fork is aim to keep as
much as possible from the original code, but make it as simple as possible. 

#### Docker

```bash
docker pull olivernadj/elasticsearch-light-exporter:latest
docker run --rm -p 9114:9114 olivernadj/elasticsearch-light-exporter:latest
```
...
| es.path                 | 1.0.5                 | Path of [Index stats API](https://www.elastic.co/guide/en/elasticsearch/reference/current/indices-stats.html) | /_all/_stats |
...
### Possible Metrics
Some or many may miss depends on `es.path` settings. 
